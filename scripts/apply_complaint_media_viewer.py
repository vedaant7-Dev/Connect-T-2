...
Path("mobile/components/ComplaintMediaViewer.tsx").write_text(component)

replace(
    "mobile/app/complaint/[id].tsx",
    'import { Feather } from "@expo/vector-icons";\n',
...
replace(
    "mobile/app/super-admin/index.tsx",
...
expo = app["expo"]
expo["version"] = "1.0.4"
expo["android"]["versionCode"] = 5
expo["ios"].setdefault("infoPlist", {})["NSPhotoLibraryAddUsageDescription"] = "Connect T saves complaint images and videos to your media library only when you choose Save."
plugins = expo.setdefault("plugins", [])
if "expo-video" not in plugins:
    plugins.insert(-2 if len(plugins) >= 2 else len(plugins), "expo-video")
if "expo-media-library" not in plugins:
    plugins.insert(-2 if len(plugins) >= 2 else len(plugins), "expo-media-library")
...
Path("mobile/test/complaint-media-viewer.test.mjs").write_text(r'''import assert from "node:assert/strict";
