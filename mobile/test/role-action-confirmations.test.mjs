...
test("Super Admin deactivate and remove actions use the shared cross-platform confirmation modal", () => {
  ...
  assert.match(screen, /setPendingAction\(\{ kind: "status"/);
  assert.match(screen, /setPendingAction\(\{ kind: "remove"/);
...
test("Nagarsevak deactivate and revoke actions require confirmation and refresh the roster", () => {
  ...
  assert.match(screen, /ConfirmActionModal/);
  assert.match(screen, /setPendingAction\(\{ item, status:/);
...
test("shared confirmation modal supports action-specific icons and blocks duplicate submission", () => {
