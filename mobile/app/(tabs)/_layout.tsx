    ...
    <Animated.View style={[{ position: "absolute", bottom: 0, left: 0, right: 0, height: TAB_HEIGHT }, tabBarAnimatedStyle]}>
      ...
      <View style={{ flex: 1, flexDirection: "row", alignItems: "center", paddingBottom: BOTTOM_INSET }}>
        {state.routes.map((route: any, index: number) => {
          ...
          return <TouchableOpacity key={route.key} onPress={onPress} onLongPress={onLongPress} activeOpacity={0.8} style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingTop: 6, paddingHorizontal: 2 }} accessibilityRole="tab" accessibilityState={{ selected: isFocused }} accessibilityLabel={label}>{route.name === "complaints" ? <ComplaintBubbleIcon color={tintColor} size={22} /> : <Feather name={(iconMap[route.name] || "circle") as any} size={22} color={tintColor} />}<Text numberOfLines={2} style={{ fontSize: 11, lineHeight: 14, fontWeight: "700", fontFamily: "Inter_700Bold", marginTop: 3, color: tintColor, textAlign: "center" }}>{label}</Text></TouchableOpacity>;
        })}
      </View>
    </Animated.View>
  );
...
function NagarsevakTabBar({ state, descriptors, navigation, labels }: any) {
  ...
  const labelMap: Record<string, string> = { admin: labels.home, ward: labels.ward, news: labels.news, profile: labels.profile };
  ...
  return (
    <View style={{ flexDirection: "row", backgroundColor: "white", paddingBottom: bottomInset, paddingTop: 7, borderTopWidth: 1, borderTopColor: "#E2E8F0", shadowColor: "#166534", shadowOpacity: 0.08, shadowRadius: 12, shadowOffset: { width: 0, height: -4 }, elevation: 10 }}>
      {orderedNames.map((name) => {
        ...
        return <TouchableOpacity key={route.key} onPress={onPress} style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingVertical: 3, paddingHorizontal: 2 }} activeOpacity={0.72} accessibilityRole="tab" accessibilityLabel={label} accessibilityState={{ selected: isFocused }}><View style={{ width: 42, height: 32, alignItems: "center", justifyContent: "center", backgroundColor: isFocused ? "rgba(22,163,74,0.12)" : "transparent", borderRadius: 16, borderWidth: isFocused ? 1 : 0, borderColor: "rgba(22,163,74,0.18)" }}><Feather name={(iconMap[name] || "circle") as any} size={20} color={isFocused ? GREEN : MUTED} /></View><Text numberOfLines={2} style={{ fontSize: 10, lineHeight: 13, fontFamily: isFocused ? "Inter_700Bold" : "Inter_600SemiBold", color: isFocused ? GREEN : MUTED, marginTop: 2, textAlign: "center" }}>{label}</Text></TouchableOpacity>;
      })}
    </View>
  );
}
...
export default function TabLayout() {
  ...
  const nagarsevakLabels = { home: t("home"), ward: t("ward"), news: t("news"), profile: t("profile") };

  return (
    <Tabs backBehavior="history" initialRouteName={isNagarsevak ? "admin" : "index"} tabBar={(props) => isNagarsevak ? <NagarsevakTabBar {...props} labels={nagarsevakLabels} /> : <AnimatedTabBar {...props} />} screenOptions={{ headerShown: false, tabBarActiveTintColor: isNagarsevak ? GREEN : ORANGE, tabBarInactiveTintColor: MUTED }}>
      <Tabs.Screen name="admin" options={{ title: nagarsevakLabels.home, href: isNagarsevak ? undefined : null }} />
      <Tabs.Screen name="ward" options={{ title: nagarsevakLabels.ward, href: isNagarsevak ? undefined : null }} />
      <Tabs.Screen name="news" options={{ title: nagarsevakLabels.news, href: isNagarsevak ? undefined : null }} />
