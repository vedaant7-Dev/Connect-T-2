  ...
  return (
    <View style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={[styles.statusPill, { backgroundColor: item.status === "scheduled" ? "#FEF3C7" : "#DCFCE7" }]}>
          <Text style={[styles.statusText, { color: item.status === "scheduled" ? "#B45309" : "#166534" }]}>{item.status === "scheduled" ? "Scheduled" : "Sent"}</Text>
        </View>
      </View>
      <Text style={styles.cardBody}>{item.body}</Text>
      {item.mediaUri ? item.mediaType === "image" ? (
        <Image source={{ uri: item.mediaUri }} style={styles.cardImage} resizeMode="cover" />
      ) : (
        <TouchableOpacity style={styles.videoRow} onPress={() => void Linking.openURL(item.mediaUri!)} accessibilityRole="button">
          ...
          <View style={styles.videoCopy}><Text style={styles.videoTitle}>Attached video</Text><Text style={styles.videoMeta}>{item.mediaDurationSeconds ? `${item.mediaDurationSeconds} seconds` : "Up to 5 minutes"}</Text></View>
      ...
      <View style={styles.cardFooter}>
        ...
        <TouchableOpacity style={styles.archiveButton} onPress={onArchive}><Feather name="archive" size={14} color="#64748B" /><Text style={styles.archiveText}>Archive</Text></TouchableOpacity>
      </View>
    </View>
  );
}
...
export default function BroadcastCenterMediaScreen() {
  ...
  const stats = useMemo(() => ({
    ...
    withMedia: active.filter((item) => !!item.mediaUri).length,
  ...
  const confirmArchive = (item: AppBroadcast) => Alert.alert(
    "Archive broadcast?",
    `Archive “${item.title}”? Delivery history will remain available.`,
    [
    ...
    <View style={styles.root}>
      <LinearGradient colors={["#052E16", "#166534", GREEN]} style={[styles.header, { paddingTop: (Platform.OS === "web" ? 54 : insets.top) + 10 }]}>
        <View style={styles.headerRow}>
          ...
          <TouchableOpacity style={styles.createButton} onPress={() => setComposeVisible(true)}><Feather name="plus" size={15} color="#166534" /><Text style={styles.createText}>Create</Text></TouchableOpacity>
        </View>
        <Text style={styles.headerTitle}>Broadcast Center</Text>
        <Text style={styles.headerSub}>Send immediate or scheduled in-app updates with an optional image or five-minute video.</Text>
        <View style={styles.statsRow}>
          <Stat value={stats.sent} label="Sent" />
          <Stat value={stats.scheduled} label="Scheduled" />
          <Stat value={stats.withMedia} label="Media" />
      ...
      <AppScrollView contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 12) + 28 }]} onAppRefresh={() => refreshBroadcasts()}>
        <View style={styles.infoBanner}><Feather name="shield" size={17} color="#1D4ED8" /><Text style={styles.infoText}>Images are validated up to 10MB. MP4/MOV videos are validated on the server and cannot exceed 5 minutes or 50MB.</Text></View>
        ...
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <View style={styles.sheet}>
            ...
            <AppScrollView contentContainerStyle={styles.formContent} automaticallyAdjustKeyboardInsets keyboardShouldPersistTaps="handled">
              ...
              <Label text="ATTACHMENT (OPTIONAL)" />
              ...
              {isSuperAdmin ? <>
                <Label text="AUDIENCE" />
                <View style={styles.choiceWrap}>{AUDIENCES.map((item) => <TouchableOpacity key={item.key} style={[styles.choice, audienceRole === item.key && styles.choiceActive]} onPress={() => setAudienceRole(item.key)}><Text style={[styles.choiceText, audienceRole === item.key && styles.choiceTextActive]}>{item.label}</Text></TouchableOpacity>)}</View>
                <Label text="WARD" />
                <TouchableOpacity style={[styles.input, styles.picker]} onPress={() => setWardPickerVisible(true)}><Text style={styles.pickerText}>{ward}</Text><Feather name="chevron-down" size={16} color="#64748B" /></TouchableOpacity>
              </> : <View style={styles.scopeBanner}><Feather name="shield" size={14} color="#166534" /><Text style={styles.scopeText}>Nagarsevak broadcasts are limited to citizens in {user?.ward || "the assigned ward"}.</Text></View>}
              ...
              <Text style={styles.help}>Leave blank to send immediately. Scheduled broadcasts require a future time.</Text>
      ...
      <Modal visible={wardPickerVisible} transparent animationType="slide" onRequestClose={() => setWardPickerVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  createButton: { minHeight: 42, flexDirection: "row", alignItems: "center", gap: 6, borderRadius: 13, backgroundColor: "white", paddingHorizontal: 13 },
  createText: { color: "#166534", fontSize: 12, fontFamily: "Inter_700Bold" },
  ...
  infoBanner: { flexDirection: "row", alignItems: "flex-start", gap: 9, padding: 12, borderRadius: 15, backgroundColor: "#EFF6FF", borderWidth: 1, borderColor: "#BFDBFE" },
  infoText: { flex: 1, color: "#1D4ED8", fontSize: 10.5, lineHeight: 16, fontFamily: "Inter_400Regular" },
  ...
  empty: { padding: 30, alignItems: "center", borderRadius: 20, backgroundColor: "white", borderWidth: 1, borderColor: "#E2E8F0" },
  emptyTitle: { marginTop: 10, color: "#0F172A", fontSize: 16, fontFamily: "Inter_700Bold" },
  emptyText: { marginTop: 5, color: "#64748B", fontSize: 11.5, textAlign: "center", fontFamily: "Inter_400Regular" },
  card: { padding: 14, borderRadius: 18, backgroundColor: "white", borderWidth: 1, borderColor: "#E2E8F0" },
  ...
  cardImage: { marginTop: 10, width: "100%", height: 180, borderRadius: 14, backgroundColor: "#F1F5F9" },
  videoRow: { marginTop: 10, minHeight: 64, borderRadius: 14, backgroundColor: "#FFF7ED", borderWidth: 1, borderColor: "#FED7AA", flexDirection: "row", alignItems: "center", paddingHorizontal: 12, gap: 10 },
  videoCopy: { flex: 1 },
  videoTitle: { color: "#9A3412", fontSize: 11.5, fontFamily: "Inter_700Bold" },
  videoMeta: { marginTop: 2, color: "#C2410C", fontSize: 9.5, fontFamily: "Inter_400Regular" },
  metrics: { marginTop: 12, flexDirection: "row", gap: 6 },
  metric: { flex: 1, minHeight: 52, borderRadius: 12, backgroundColor: "#F8FAFC", alignItems: "center", justifyContent: "center" },
  metricValue: { color: "#0F172A", fontSize: 15, fontFamily: "Inter_700Bold" },
  providerValue: { color: "#B45309", fontSize: 10.5 },
  metricLabel: { marginTop: 2, color: "#94A3B8", fontSize: 8.2, fontFamily: "Inter_500Medium" },
  ...
  archiveButton: { minHeight: 40, flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9 },
  ...
  sheetHeader: { minHeight: 64, flexDirection: "row", alignItems: "center", paddingHorizontal: 18, borderBottomWidth: 1, borderBottomColor: "#E2E8F0" },
  ...
  previewTitle: { marginTop: 8, color: "#0F172A", fontSize: 14, fontFamily: "Inter_700Bold" },
