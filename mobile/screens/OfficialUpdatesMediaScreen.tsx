  ...
  return (
    <TouchableOpacity style={[styles.card, !item.isRead && styles.unreadCard]} activeOpacity={0.86} onPress={onOpen} accessibilityRole="button" accessibilityLabel={`${config.label}: ${item.title}`}>
      {item.media?.type === "image" ? (
        <View style={styles.mediaWrap}><Image source={{ uri: item.media.uri }} style={styles.thumbnail} />{!item.isRead ? <View style={styles.unreadDot} /> : null}</View>
      ) : (
        <View style={[styles.iconBox, { backgroundColor: config.bg }]}><Feather name={item.media?.type === "video" ? "play-circle" : config.icon} size={23} color={config.color} />{!item.isRead ? <View style={styles.unreadDot} /> : null}</View>
      )}
  ...
  return (
    <TouchableOpacity style={[styles.card, !item.isRead && styles.unreadCard]} activeOpacity={0.86} onPress={onOpen} accessibilityRole="button" accessibilityLabel={`${label}: ${item.title}`}>
      {item.mediaType === "image" && item.mediaUri ? (
        <View style={styles.mediaWrap}><Image source={{ uri: item.mediaUri }} style={styles.thumbnail} />{!item.isRead ? <View style={styles.unreadDot} /> : null}</View>
      ) : (
        <View style={[styles.iconBox, { backgroundColor: config.bg }]}><Feather name={item.mediaType === "video" ? "play-circle" : config.icon} size={23} color={config.color} />{!item.isRead ? <View style={styles.unreadDot} /> : null}</View>
      )}
      <View style={styles.cardContent}>
...
export default function OfficialUpdatesMediaScreen() {
  ...
  const openBroadcast = async (item: AppBroadcast) => {
    ...
    if (!item.isRead) {
      try {
        ...
        // Keep the message readable; the next refresh retries the receipt sync.
      }
    }
  };
      ...
      <Modal visible={!!selectedBroadcast} transparent animationType="fade" onRequestClose={() => setSelectedBroadcast(null)}>
        <View style={styles.modalOverlay} accessibilityViewIsModal>
          <View style={styles.detailModal}>
            <View style={styles.detailIcon}><Feather name={selectedBroadcast?.mediaType === "video" ? "play-circle" : "radio"} size={25} color="#B45309" /></View>
            ...
            {selectedBroadcast?.mediaType === "image" && selectedBroadcast.mediaUri ? <Image source={{ uri: selectedBroadcast.mediaUri }} style={styles.detailImage} resizeMode="contain" /> : null}
            {selectedBroadcast?.mediaType === "video" && selectedBroadcast.mediaUri ? <TouchableOpacity style={styles.playButton} onPress={() => void Linking.openURL(selectedBroadcast.mediaUri!)} accessibilityRole="button"><Feather name="play-circle" size={20} color="white" /><Text style={styles.playText}>Play attached video</Text></TouchableOpacity> : null}
...
const styles = StyleSheet.create({
  ...
  errorText: { flex: 1, fontSize: 10.5, lineHeight: 15, color: "#92400E", fontFamily: "Inter_500Medium" },
  ...
  thumbnail: { width: 58, height: 58, borderRadius: 15, backgroundColor: "#F8FAFC" },
  ...
  mediaHint: { marginTop: 6, color: ORANGE, fontSize: 9.3, fontFamily: "Inter_600SemiBold" },
  ...
  detailModal: { width: "100%", maxWidth: 420, maxHeight: "90%", borderRadius: 24, backgroundColor: "white", padding: 22, alignItems: "center" },
  ...
  detailImage: { marginTop: 13, width: "100%", height: 230, borderRadius: 15, backgroundColor: "#F8FAFC" },
  playButton: { marginTop: 13, width: "100%", minHeight: 50, borderRadius: 14, backgroundColor: ORANGE, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  playText: { color: "white", fontSize: 12, fontFamily: "Inter_700Bold" },
