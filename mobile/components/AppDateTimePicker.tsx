...
<View style={styles.calendarGrid}>{calendarCells.map((date, index) => {
  ...
  return <TouchableOpacity key={date.toISOString()} style={[styles.dayCell, active && styles.dayCellActive, today && !active && styles.todayCell]} disabled={disabled} onPress={() => chooseDate(date)}><Text style={[styles.dayCellText, active && styles.dayCellActiveText, disabled && styles.disabledText]}>{date.getDate()}</Text></TouchableOpacity>;
})}</View>
